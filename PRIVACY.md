...
- **Local files only.** All state lives in files you can read: the config (`~/.claude/.coalface.json` global, plus an optional per-project config under an agent dir — `.claude/coal/coalface.json`, `.agents/coal/coalface.json`, `.gemini/coal/coalface.json`, or one of the two deprecated legacy shapes — nested `.claude/.coalface.json`, root `.coalface.json` — still read; see README § Deprecated config paths) and the self-update throttle stamp `~/.claude/coal/coalface/update-check` (a timestamp, nothing more; a pre-migration stamp at the old `~/.claude/.coalface-update-check` path is still read once and then replaced).
