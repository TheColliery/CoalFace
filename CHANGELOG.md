...
### Fixed
- **`resolveCap` upward-override defeated the machine bound (CWK-034 item 1, U12 CB-audit MED-1).** `scripts/lib/admission-control.mjs`'s `resolveCap()` returned a configured `maxLocalWorkers` value OUTRIGHT when set, never clamped against `deriveMachineCap()` — a clone-borne `.coalface.json` with `maxLocalWorkers: 64` sized the admission gate at 64 even on a box whose derived cap is 1, directly violating the umbrella's THE MACHINE BOUND law (this file is the flock's own cited exemplar for it). Fixed to `Math.min(configured, machineCap)` — a config value may LOWER the cap, never raise it past what the host can run. The prior test only exercised the lowering direction (3 vs a machine cap of 7 — passed under old and new code alike); added a second case (`resolveCap(64, {cpuCount:2})` must be `1`) proven red-first against the old code by hand-executing the old branch. Verified load-bearing end to end, not just in isolation: a live throwaway script confirmed `createAdmissionGate(resolveCap(64,{cpuCount:2}))` genuinely caps real concurrent `acquire()`s at 1, queuing the other 9 of 10 simulated workers.
- **Admission-control rationale misattributed the CPU-bound gate slot to workers (CWK-034 item 2, U12 MED-2 — README:132 vs :138 self-contradiction).** Every rationale site described the bound as protecting "a worker's LOCAL gate step (the build/test run its own contract tells it to execute)" — false: CoalFace workers are spawn-tool-less Explore-class leaves that read + propose text and cannot run a build/test step. The true mechanism, already correct at README:138 and left untouched, is that the bound protects concurrent apply-time DOMAIN-GATE runs held by MAIN (or a depth-1 nested conductor for its own scope). Reconciled every misattributed site: `admission-control.mjs`'s top comment, `config-schema.mjs`'s `maxLocalWorkers` help text, `README.md` (the WAVES flow-table row and the config-key table row), `SKILL.md`'s WAVES step, all four paragraphs of `references/admission-control.md`, and — found by INSPECT, outside the unit's original scope — `platform-configs/.coalface.json`'s own comment (the user-facing config template, not swept by the original grep because it sits outside `skills/coalface/`). Constants unchanged (RESERVE=2, WORKER_CORE_WEIGHT=2, CEILING=16 were already right); only the WHO was wrong.
- **Bare "offline, no API keys" claim, unqualified, in three places board #85 never reached (CWK-034 item 3, U12 LOW).** Board #85 fixed README.md (:42, :166) to state the consent-gated `/coalface:update` check goes online while the hooks never do; the frontmatter `description`, `.claude-plugin/plugin.json`'s `description`, and — found during this unit, not originally named — `.claude-plugin/marketplace.json`'s plugin description all still carried the bare, unqualified claim. Brought all three to the same corrected substance (frontmatter: the short "offline by default" form, tight against its 996/1024-char budget; the two JSON descriptions: the fuller form matching README's own wording, well under their 1024 caps at 856 and 645 chars respectively). `marketplace.json` confirmed to have no `plugin/` dist twin by looking (`ls plugin/.claude-plugin/`), not assumed.
- **INSPECT fixback, same unit: a stale hand-copied test fixture (U12's own item-3 edit silently broke it).** `scripts/lib/claude-ai-trim.test.mjs`'s "the real CoalFace SKILL.md description" test hand-copied the frontmatter description as a string literal — the item-3 fix above changed the live description and nothing re-synced the copy, and no gate would ever have caught the drift (the test only asserted the trim mechanics, never compared against the live file). Fixed durably: the test now reads the live description via `desc-cap.mjs`'s own `frontmatterField()` at run time, so it cannot drift from source again.
