...
**Docs:** [thecolliery.gitbook.io/thecolliery-docs/tools/coalface](https://thecolliery.gitbook.io/thecolliery-docs/tools/coalface) *(publishing soon)*

**Part of [TheColliery](https://github.com/TheColliery/.github)** — siblings: **[CoalMine](https://github.com/TheColliery/CoalMine)** (quality canaries) · **[CoalTipple](https://github.com/TheColliery/CoalTipple)** (model/effort routing) · **[CoalBoard](https://github.com/TheColliery/CoalBoard)** (consensus board) · **[CoalHearth](https://github.com/TheColliery/CoalHearth)** (session warm-resume) · **[CoalWash](https://github.com/TheColliery/CoalWash)** (memory defrag) · **[CoalLedger](https://github.com/TheColliery/CoalLedger)** (docs health) · **[CoalGob](https://github.com/TheColliery/CoalGob)** (OS-trash delete guard, PUBLIC BETA v0.1.0-beta.1).
...
| 4 | **Waves** | Worker-wave width = floor(platform width × `bandwidth`%) — `maxLocalWorkers` never enters it. Separately, `maxLocalWorkers` is a MACHINE cap on concurrent apply-time domain-gate runs (main's own step-7 build/test, or a depth-1 nested conductor's own; auto-derived from your cores; excess queues, none denied). AIMD backoff: a 429 shrinks the next wave, clear stretches re-grow — a `bandwidth` set above your account's real capacity simply settles AT the real capacity, nothing breaks. |
...
**Activation.** `coalfaceMode: auto` (the default) lets the agent judge — any fan-out of ≥ `autoFanoutFloor` units (default 4) rides the contract; 1-3-sub ad-hoc spawns, below the default floor, keep zero ceremony. `on` scouts every prompt; `off` removes CoalFace entirely. Manual **`/coalface`** convenes it in any mode except `off`. There is no pre-pay gate: your command IS the consent — a solo run would burn a comparable **dollar** budget silently, and the wallet caps the swarm's dollar cost at ~that — so transparency arrives as the receipt.
...
| `autoFanoutFloor` | `4` | Fan-out size (units) at/above which an `auto`-mode fan-out must ride the contract; below it, 1-3-sub ad-hoc spawns keep zero ceremony. |
...
UNREADABLE: <abs path> exists but is not a readable config (<reason>); it was skipped — canonical = .claude/coal/coalface.json
```

`LEGACY:` names the project config the walk selected when it is one of the two deprecated shapes; it says where the file is, not that it parsed — an unreadable or malformed one is named but not applied. `IGNORED:` names a file that looks like a config but sits at a path the walk never reads, so it is not being applied — `<project>/coalface.json` (no dot), `.claude/coalface.json` (no `coal/`), or `.claude/coal/.coalface.json` (old name inside the canonical dir). Only directory levels the walk reads are probed. `UNREADABLE:` names a config that exists at a candidate path — including the global `~/.claude/.coalface.json`, which is always probed but never itself the walk's selection — but could not be read as a config — `<reason>` is `malformed JSON`, `a directory`, or `unreadable` (a permission failure, `EACCES` or a Windows ACL denial); the walk's own selection is unchanged, the candidate that IS selected still wins and still contributes nothing, only the silence goes. A correctly placed config, or none at all, prints nothing — even with `coalfaceMode: off`. All three lines still print under `coalfaceMode: off`; each names a file to move (`LEGACY:`), move or remove (`IGNORED:`), or fix (`UNREADABLE:`), and resolving that ends the line. On Antigravity the same lines ride the adapter's one message.
