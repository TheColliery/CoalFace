...
| 4 | **Waves** | Effective width = floor(platform width × `bandwidth`%), floored again by `maxLocalWorkers` — a MACHINE cap on workers holding a local build/test gate slot (auto-derived from your cores; excess queues, none denied). AIMD backoff: a 429 shrinks the next wave, clear stretches re-grow — a `bandwidth` set above your account's real capacity simply settles AT the real capacity, nothing breaks. |
...
| `maxLocalWorkers` | `0` | Ceiling on concurrent workers holding a LOCAL CPU-bound gate slot (a build/test run) — `0` auto-derives from your machine's core count; a positive integer overrides it. Excess workers queue, never denied. |
