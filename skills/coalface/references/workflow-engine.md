...
2. **Split the two failure classes in-script** — `agent()` returns `null` for a dead worker either way; the SHAPE tells them apart:
   - **Scattered nulls** (1-2, isolated) = transient (server error / stalled stream) → ONE in-script retry pass over just the nulls: `const retry = items.filter((_, i) => results[i] === null)` — `null` only, never falsy (a completed worker can return an empty string).
