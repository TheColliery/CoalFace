...
| write | Step 7 single-writer APPLY (O4, the real tree) | `Write`·`Edit`; `Bash` for the pre-swarm SNAPSHOT (O5/P16: `git stash`/HEAD-record, or a file-copy) and the end-of-step DOMAIN GATE (build+test) | `Write`/`Edit` denied → report + courier the accepted orders to the dispatcher, never claim applied. `Bash` denied → STOP before applying: no snapshot means no rollback net (P16/P18), no gate means nothing to report green — say so in the receipt (O1); never apply un-snapshotted orders, never report an unrun gate as passed |
